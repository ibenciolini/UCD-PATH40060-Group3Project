...
clean_id <- function(x) {
  x <- toupper(trimws(x))
  x <- sub("-T$", "", x)   # remove trailing -T (CPTAC mismatch fix)
  x
}

file <- file.path(DATA_DIR, "data_mutations.txt")

# Mutations
mut_mat <- read.delim(
  file,
  comment.char = "#",   # skips metadata lines
  sep = "\t",
  header = TRUE,
  fill = TRUE,
  check.names = FALSE,
...
stopifnot(all(c("Hugo_Symbol", "Tumor_Sample_Barcode") %in% colnames(mut_mat)))
...
mut_df[, Hugo_Symbol := trimws(Hugo_Symbol)]
mut_df[, Tumor_Sample_Barcode := clean_id(Tumor_Sample_Barcode)]
mut_df[, mut := 1]

mut_df[, `:=`(
  Hugo_Symbol = trimws(Hugo_Symbol),
  Tumor_Sample_Barcode = clean_id(Tumor_Sample_Barcode),
  mut = 1
)]

setDT(mut_df)

mut_wide <- data.table::dcast(
  mut_df,
...
mut_mat_final <- as.matrix(mut_wide[, -1])
rownames(mut_mat_final) <- mut_wide$Tumor_Sample_Barcode

mut_mat_final <- (mut_mat_final > 0) * 1
...
cnv_disc_raw[,1] <- clean_id(cnv_disc_raw[,1])
cnv_disc_raw <- cnv_disc_raw[!duplicated(cnv_disc_raw[,1]), ]

rownames(cnv_disc_raw) <- cnv_disc_raw[,1]
cnv_disc_raw <- cnv_disc_raw[,-1, drop = FALSE]

cnv_disc <- as.matrix(cnv_disc_raw)
storage.mode(cnv_disc) <- "numeric"

cnv_disc <- t(cnv_disc)

cnv_disc <- cnv_disc[!duplicated(rownames(cnv_disc)), , drop = FALSE]

rownames(cnv_disc) <- clean_id(rownames(cnv_disc))

cnv_disc <- cnv_disc[rownames(cnv_disc) %in% common_samples, , drop = FALSE]

# CNV LOG2
cnv_log2_raw <- read.delim(file.path(DATA_DIR, "data_linear_cna.txt"), check.names = FALSE)

cnv_log2_raw[,1] <- clean_id(cnv_log2_raw[,1])
cnv_log2_raw <- cnv_log2_raw[!duplicated(cnv_log2_raw[,1]), ]

rownames(cnv_log2_raw) <- cnv_log2_raw[,1]
cnv_log2_raw <- cnv_log2_raw[,-1, drop = FALSE]

cnv_log2 <- as.matrix(cnv_log2_raw)
storage.mode(cnv_log2) <- "numeric"

cnv_log2 <- t(cnv_log2)

cnv_log2 <- cnv_log2[!duplicated(rownames(cnv_log2)), , drop = FALSE]
rownames(cnv_log2) <- clean_id(rownames(cnv_log2))

cnv_log2 <- cnv_log2[rownames(cnv_log2) %in% common_samples, , drop = FALSE]

# RNA Expression
rna_raw <- read.delim(file.path(DATA_DIR, "data_mrna_seq_v2_rsem.txt"), check.names = FALSE)

rna_raw[,1] <- clean_id(rna_raw[,1])
rna_raw <- rna_raw[!duplicated(rna_raw[,1]), ]

rownames(rna_raw) <- rna_raw[,1]
rna_raw <- rna_raw[,-1, drop = FALSE]

rna_mat <- as.matrix(rna_raw)
storage.mode(rna_mat) <- "numeric"

rna_mat <- t(rna_mat)

rna_mat <- rna_mat[!duplicated(rownames(rna_mat)), , drop = FALSE]
rownames(rna_mat) <- clean_id(rownames(rna_mat))

rna_mat <- rna_mat[rownames(rna_mat) %in% common_samples, , drop = FALSE]
...
mut_freq <- apply(mut_mat, 2, function(g) {
  tapply(g, cl_vec, mean, na.rm = TRUE)
})

mut_freq <- as.data.frame(mut_freq)
mut_freq$gene <- rownames(mut_freq)

mut_enrichment <- list()

for (g in colnames(mut_mat)) {

  tmp <- list()

  for (cl in unique(cl_vec)) {

    tab <- table(mut_mat[, g] > 0, cl_vec == cl)
  ...
  if (length(tmp) > 0) {
    mut_enrichment[[length(mut_enrichment) + 1]] <- do.call(rbind, tmp)
  }
}
...
cnv_var <- apply(cnv_log2_clean, 2, function(x) var(x, na.rm = TRUE))
...
for (j in seq_len(ncol(cnv_log2_clean))) {
  ...
  cnv_log2_clean[, j] <- col
}
cnv_pca <- prcomp(cnv_log2_clean, scale. = TRUE)

cnv_df <- as.data.frame(cnv_pca$x[, 1:5])
cnv_df$cluster <- cluster_labels[rownames(cnv_log2_clean)]

ggplot(cnv_df, aes(PC1, PC2, colour = cluster)) +
  geom_point(size = 2.5) +
  theme_bw() +
...
cnv_aligned <- cnv_log2[common_samples, , drop = FALSE]
...
cnv_prot_cor_df <- data.frame(
  gene = names(cnv_prot_cor),
  cor = cnv_prot_cor
)

ggplot(cnv_prot_cor_df, aes(x = cor)) +
...
cnv_disc_long <- reshape(
  cnv_disc_long,
  varying = colnames(cnv_disc),
  v.names = "cnv_state",
  timevar = "gene",
  times = colnames(cnv_disc),
  direction = "long"
)

cnv_disc_long$cluster <- cluster_labels[cnv_disc_long$sample_id]

cnv_enrich <- aggregate(
  cnv_state ~ cluster + gene,
  data = cnv_disc_long,
...
rna_df$cluster <- cluster_labels[rownames(rna_z)]

ggplot(rna_df, aes(PC1, PC2, colour = cluster)) +
  geom_point(size = 2) +
  theme_bw() +
...
rna_aligned <- rna_z[common_samples, , drop = FALSE]
prot_aligned <- prot_imputed[common_samples, , drop = FALSE]
...
for (cl in unique(cl_vec)) {
  ...
  fit <- lmFit(t(rna_cl), design)
  cont <- makeContrasts(Target - Other, levels = design)
  ...
  rna_de[[length(rna_de) + 1]] <- tt
}

rna_de_all <- do.call(rbind, rna_de)
```
...
surv_multi <- cluster_df
surv_multi$mut_burden <- mut_burden[surv_multi$sample_id]
surv_multi$cnv_instability <- cnv_instability[surv_multi$sample_id]

surv_multi <- surv_multi[!is.na(surv_multi$os_months), ]

cox_multi <- coxph(
  Surv(os_months, os_event) ~ cluster +
    mut_burden +
    cnv_instability +
    age,
  data = surv_multi
